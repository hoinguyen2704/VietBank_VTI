...
public class AccountController {
    ...
    public ResponseEntity<ApiResponse<PageResponse<AccountResponse>>> getAllAccounts(
        ...
        if (status != null) {
            filterRequest.setStatus(com.vti.vietbank2.entity.enums.AccountStatus.valueOf(status.toUpperCase()));
        }
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse<Void>> closeAccount(@PathVariable Integer id) {
        return ResponseEntity.ok(accountService.closeAccount(id));
    }
    
    @GetMapping("/customer/{customerId}")
    public ResponseEntity<ApiResponse<List<AccountResponse>>> getByCustomerId(@PathVariable Integer customerId) {
        return ResponseEntity.ok(accountService.getByCustomerId(customerId));
    }
    
    @GetMapping("/{id}/balance")
    public ResponseEntity<ApiResponse<BigDecimal>> getBalanceById(@PathVariable Integer id) {
